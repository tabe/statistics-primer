...
つまり、不確かな状況を表す確率変数に応じた決定によって損失をなるべく小さくすることを目的としている。
...
Kulesa et al. (2015)[fn:7]では、variance-to-mean ratio (VMR)を推定する統計量の標本分布についての情報を得るという題材で、ノンパラメトリックおよびパラメトリックなブートストラップ法を用いる例が紹介されている。
