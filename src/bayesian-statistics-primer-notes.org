...
このことは、パラメトリックモデルを用いて仮説を表す場合には、次のように言い換えられる。

- 頻度主義統計におけるパラメトリックモデルでは、その確率分布の母数は（未知の）固定された値をもつとする。このため、この母数の値を明らかにしようとする。
- 一方、ベイズ統計におけるパラメトリックモデルでは、その母数を別の確率変数で表す。このため、この確率変数が従う分布を明らかにしようとする。
...
6面体サイコロが1つ与えられる。
\begin{itemize}
\item \(\Theta = [0, 1]^6\): あるサイコロの出目の分布を表すカテゴリカル分布の母数
...
6面体サイコロが1つ与えられる。
\begin{itemize}
\item \(\Theta = [0, 1]^6\): そのサイコロの出目の分布を表すカテゴリカル分布の母数
\item \(\Realization = \{1,2,3,4,5,6\}^n\): そのサイコロを\(n\)回独立に振った場合の出目を表す長さ\(n\)の列
...
しかし、 _実際にデータとして用いるのは\(X\)からの\(n\)個の独立な標本_ であり、意思決定は標本分布に従う別の確率変数\(S\)を通じて行われる 。
...
母平均の推定量としての標本平均は、不偏性、一致性、および有効性という3つの性質を兼ね備えた理想的な推定量である。
その普遍性については、期待値の線形性からすぐ分かる。
その一致性については、[[https://en.wikipedia.org/wiki/Law_of_large_numbers][大数の法則]]から分かる。
さらに、[[https://en.wikipedia.org/wiki/Central_limit_theorem][中央極限定理]]により、\(X\)が正規分布に従わなくても、上の\(s\)の標本分布は漸近的に正規分布することが分かる[fn:5]。
...
** 階層ベイズモデルの例: 肉腫の治療効果
肉腫に対するimatinibの治療効果に関する第II相臨床試験で得られたデータを階層ベイズモデルで解析する例が、Thall et al. (2003)[fn:6]で紹介されている。
以下にそのモデルを概説する。
...
Rの =trialr= パッケージのドキュメント[fn:7]には、MCMC法による解析例が示されている。
...
Kulesa et al. (2015)[fn:8]では、variance-to-mean ratio (VMR)の推定量の標本分布についての情報を得るという題材で、ノンパラメトリックおよびパラメトリックなブートストラップ法を用いる例が紹介されている。
...
- 推定量[estimator] :: 統計量の一種。母数を推定するため観察された実現値から計算される値、あるいはその導出規則（式やアルゴリズム）。
...
- ベルヌーイ分布[Bernoulli distribution] :: 2つの値のうちのどちらかを取る試行を表す確率分布。コイン投げが典型例。
- カテゴリカル分布[categorical distribution] :: k個の値を取りうる試行を表す確率分布。サイコロの出目が典型例。ベルヌーイ分布を一般化したものと考えられる。
- 不偏性[unbiasness] :: 推定量の性質の1つ。母数\(\theta\)の推定量の期待値が\(\theta\)と等しいこと。
- 一致性[consistency] :: 推定量の性質の1つ。母数\(\theta\)の推定量は標本サイズ\(n\)に依存するが、\(n\)が増えるにつれ\(\theta\)に収束すること。
- 有効性[efficiency] :: 推定量の性質の1つ。母数\(\theta\)の推定量が効率的とは、他のどの\(\beta\)の推定量より分散が等しいかより小さいこと。
...
[fn:5] ただし、\(X\)が（有限の）平均と分散をもつことを前提としている。

[fn:6] Thall PF, Wathen JK, Bekele BN, Champlin RE, Baker LH, Benjamin RS. Hierarchical Bayesian approaches to phase II trials in diseases with multiple subtypes. Statistics in Medicine. 2003;22(5):763-780. doi:10.1002/sim.1399

[fn:7] https://brockk.github.io/trialr/articles/A500-HierarchicalBayesianResponse.html

[fn:8] Kulesa, A., Krzywinski, M., Blainey, P. & Altman, N. Sampling distributions and the bootstrap. Nature Methods 12, 477–478 (2015). doi:10.1038/nmeth.3414
