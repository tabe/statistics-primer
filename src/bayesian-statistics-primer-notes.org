...
\begin{align}
p_i &= \operatorname{logit}^{-1}(\rho_i);\\
x_i &\sim \operatorname{Binomial}(n_i, p_i)
\end{align}

臨床試験の報告データ\(D\)に基づいて治療に効果が期待できるかどうかは、以下のように尤度の大きさを評価することで判断できる。
\begin{align}
\CondProb{p_i > \theta}{D} > q
\end{align}
ただし\(\theta\)は考慮に値する応答率の下限、\(q\)は承認に必要な確からしさを表す。
Thall et al. (2003)では、\(\theta = 0.3\)とされた。
